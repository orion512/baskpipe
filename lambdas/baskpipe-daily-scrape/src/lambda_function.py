...
def list_dicts_to_s3(data: list[dict], s3_path: str):
    ...
    s3_path (str): The full S3 path in the format 's3://bucket-name/path/to/file.csv' where the CSV will be stored.
...
def lambda_handler(event, context):
    ...
    Validates the presence and format of 'date' and 's3_path' in the event object.
    Scrapes games data for the given date and stores the results in S3.
    ...
    Raises:
    ValueError: If 'date' or 's3_path' are missing or if 'date' is not in the correct format.
    Exception: If an error occurs during the scraping or S3 writing process.
    """
    ...
    try:
        ...
        print(err)
        raise Exception('Something went wrong with writing data to S3.')
